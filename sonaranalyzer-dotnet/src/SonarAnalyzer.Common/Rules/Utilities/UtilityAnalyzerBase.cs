...
namespace SonarAnalyzer.Rules
{
    public abstract class UtilityAnalyzerBase : SonarDiagnosticAnalyzer
    {
        ...
        protected Dictionary<string, bool> AnalyzeGeneratedCode { get; } = new Dictionary<string, bool>
            {
                { AnalyzeGeneratedCodeCSharp, false },
                { AnalyzeGeneratedCodeVisualBasic, false },
            };
        ...
        protected void ReadParameters(AnalyzerOptions options, string language)
        {
            ...
            lock (this.parameterReadLock)
            {
                ...
                ReadGeneratedCodeProperties(settings);
        ...
        {
            IgnoreHeaderComments[IgnoreHeaderCommentsCSharp] = ReadProperty(settings, IgnoreHeaderCommentsCSharp);
            IgnoreHeaderComments[IgnoreHeaderCommentsVisualBasic] = ReadProperty(settings, IgnoreHeaderCommentsVisualBasic);
        }

        private void ReadGeneratedCodeProperties(IEnumerable<XElement> settings)
        {
            AnalyzeGeneratedCode[AnalyzeGeneratedCodeCSharp] = ReadProperty(settings, AnalyzeGeneratedCodeCSharp);
            AnalyzeGeneratedCode[AnalyzeGeneratedCodeVisualBasic] = ReadProperty(settings, AnalyzeGeneratedCodeVisualBasic);
        }

        private bool ReadProperty(IEnumerable<XElement> settings, string propertyName)
        {
        ...
        where TMessage : IMessage, new()
    {
                ...
                c =>
                {
                    ...
                    foreach (var tree in c.Compilation.SyntaxTrees)
                    {
                        if (ShouldGenerateMetrics(tree) &&
                            (AnalyzeAutogeneratedCode(c.Compilation.Language) || !GeneratedCodeRecognizer.IsGenerated(tree)))
                        {
        ...
        protected virtual bool AnalyzeAutogeneratedCode(string language)
        {
            return AnalyzeGeneratedCode[GetProperty()];
